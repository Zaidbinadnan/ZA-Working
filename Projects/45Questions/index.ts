...
console.log(
  "Sorry, Bigger table is not arrived on time, so i've to invite only two peoples"
);
...
while (dinnerList.length > 2) {
  ...
  console.log(
    `Sorry ${removedGuest}, I'm unable to invite you now due to some issue`
  );
} // invite cancellation messee to rest
//Print a message to each of the two people still on your list,
console.log("Invite to rest two");
dinnerList.forEach((list) =>
  console.log(`Hi ${list} you are still invited to today's dinner`)
);
...
mountains_cities.forEach((mountcities) =>
  console.log("Q20 List:", mountcities)
);
...
let usernames: string[] = ["zaid", "hamdan", "jahanzaib", "sarfraz", "admin"];
...
let current_users: string[] = ["zaid", "hamdan", "jahanzaib", "waqar"];
let new_users: string[] = ["sarfraz", "adnan", "jahanzaib", "hamdan"];

new_users.forEach((new_user) => {
  if (current_users.includes(new_user.toLowerCase())) {
    console.log(
      `The username '${new_user}' is not avaiable. Please enter a new username.`
    );
  } else {
...
//Q38
console.log("Q38");
function decribe_city(city: string, country: string = "unknown"): void {
...
function make_album(
  ...
  albumTitle: string,
  albumTrack?: number
): { artistName: string; albumTitle: string; albumTrack?: number } {
  let albumObject = {
...
let magicians: string[] = ["Magicane A", "Magicane B", "Magicane C"];
...
let magicians1: string[] = ["Magicane D", "Magicane E", "Magicane F"];

// Q:Create a copy of the array and modify it to include "the Great"
let copyOfMagicians1: string[] = [...magicians1];
let greatMagicians1: string[] = make_great1(copyOfMagicians1);
